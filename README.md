...
- 宾客扫码进入上传页，拍照/选图后自动压缩并上传；
- 上传走 nginx WebDAV（`PUT /uploads/`），照片直接落到服务器；
- 新人进入相册页（密码保护），查看全部宾客上传的照片；
- 相册页内置现场二维码，可截图打印放在迎宾处。
...
└── src/
    ...
    ├── router/             # Hash 路由：/ 上传页、/gallery 相册页
    ├── storage/            # 存储适配层：生产=WebDAV，本地开发=IndexedDB
    ├── utils/              # 图片压缩、本机上传记录
    └── views/              # Upload.vue（宾客）、Gallery.vue（新人）
```
